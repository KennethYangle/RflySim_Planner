...
class Utils(object):
    ...
    def RotateAvoidancePDAccelerationController(self, pos_info, pos_i):
        K1, K2 = 0.5, 0.5
        V = np.linalg.norm(pos_info["mav_vel"])
        a_d = K1 * self.sat(pos_info["desired_pos"] - pos_info["mav_pos"], 2.0) + K2  * ((V + 1) * pos_info["desired_vel_norm"] - pos_info["mav_vel"])
